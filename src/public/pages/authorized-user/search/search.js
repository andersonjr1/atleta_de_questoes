...
async function showQuestionModal(questionId) {
    ...
    try {
        const response = await fetch(`http://localhost:4000/api/questions/search?id=${questionId}`);
        ...
        modalTitle.textContent = `Questão ${questionData.vestibular || ''} ${questionData.year || ''}`;
        ...
        if(questionData.text) {
            ...
            questionText.innerHTML = `<p>${questionData.text}</p>`;
            ...
            questionData.alternatives.forEach(alternative => {
                ...
                altDiv.addEventListener('click', () => {
                    ...
                    if (alternative.is_correct) {
                        ...
                        const correctAlternative = questionData.alternatives.find(a => a.is_correct);
                        if (correctAlternative) {
                            const correctIndex = questionData.alternatives.indexOf(correctAlternative);
        ...
        metadataDiv.innerHTML = `
            ...
            <div style="display: grid; grid-template-columns: max-content 1fr; gap: 10px 20px;">
                ${questionData.discipline ? `<span style="font-weight: bold;">Disciplina:</span><span>${questionData.discipline}</span>` : ''}
                ${questionData.sub_discipline ? `<span style="font-weight: bold;">Subárea:</span><span>${questionData.sub_discipline}</span>` : ''}
                ${questionData.year ? `<span style="font-weight: bold;">Ano:</span><span>${questionData.year}</span>` : ''}
                ${questionData.level ? `<span style="font-weight: bold;">Nível:</span><span>${'⭐'.repeat(questionData.level)}</span>` : ''}
                ${questionData.vestibular ? `<span style="font-weight: bold;">Origem:</span><span>${questionData.vestibular}</span>` : ''}
            </div>
        `;
...
async function renderSearchPage() {
    ...
    const uniqueYears = [...new Set(initialData.map(q => q.year))].sort((a, b) => b - a);
    const uniqueDisciplines = [...new Set(initialData.map(q => q.discipline))];
    const uniqueSubDisciplines = [...new Set(initialData.map(q => q.sub_discipline))];
    ...
    const selectsHTML = `
        ...
        <select id="discipline-filter">
            ...
            ${uniqueDisciplines.map(discipline => `<option value="${discipline}">${discipline}</option>`).join('')}
        </select>
        
        <select id="subject-filter">
            <option value="" selected>Todos os assuntos</option>
            ${uniqueSubDisciplines.map(subject => `<option value="${subject}">${subject}</option>`).join('')}
        </select>
        
        <select id="level-filter">
            <option value="" selected>Todos os níveis</option>
            ${possibleLevels.map(level => `<option value="${level}">${'⭐'.repeat(level)}</option>`).join('')}
        </select>
    `;
...
function buildSearchPath(container) {
    const params = {
        ...
        assunto: container.querySelector('#subject-filter').value,
...
async function searchQuestions(searchPath) {
    try {
        const response = await fetch(`http://localhost:4000/api/questions${searchPath}`, {
            method: 'GET',
            headers: {
...
function addQuestionToContainer(questionData) {
    ...
    questionElement.innerHTML = `
        ...
        <div class="tags">
            ...
            <span class="tag tag-area">${questionData.discipline}</span>
            <span class="tag tag-assunto">${questionData.sub_discipline}</span>
            <span class="tag star-emoji">${'⭐'.repeat(questionData.level)}</span>
        </div>
    `;
